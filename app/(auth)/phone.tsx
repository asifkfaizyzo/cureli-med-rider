...
export default function PhoneScreen() {
  ...
  async function handleContinue() {
    ...
    try {
      ...
      if (result.exists) {
        // Existing rider → login screen (password / OTP tabs)
        router.push({ pathname: '/(auth)/login', params: { phone } });
      } else {
        // New rider → send OTP and go to OTP screen
        try {
...
const styles = StyleSheet.create({
  ...
  wordmark: { fontSize: 36, fontWeight: '800', letterSpacing: -1 },
  tagline: { fontSize: 14, letterSpacing: 1.5, textTransform: 'uppercase' },
  ...
  prefix: { paddingHorizontal: 16, fontSize: 16, fontWeight: '500' },
  divider: { width: 1, height: 24 },
  input: { flex: 1, paddingHorizontal: 16, fontSize: 18, letterSpacing: 1 },
