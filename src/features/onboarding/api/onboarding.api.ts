...
export const onboardingApi = {
  ...
  savePersonalDetails: async (payload: {
    ...
    const { data } = await api.put(
      "/rider/onboarding/personal-details",
      payload,
    );
    return data.data;
  },
  ...
  saveVehicleDetails: async (payload: {
    ...
    const { data } = await api.put(
      "/rider/onboarding/vehicle-details",
      payload,
    );
    return data.data;
  },
  ...
  // ── Submit (safety net — backend auto-submits on last upload) ──
  submit: async () => {
    const { data } = await api.post("/rider/onboarding/submit");
    return data.data;
  },

  // REMOVED: resubmit — resubmission is now automatic via upload flow
